...
fn_plot <- function(x1, y1, slices = c(1,4,6), type, u){
  ...
  if(dif != 0){
    ...
    p <- 
      ggplot() +
      geom_raster(data = tb, aes(lon, lat, fill = mean)) +
      geom_raster(data = tb_deserts, fill = "grey70", aes(lon, lat)) +
...
s <- 
  ...
  str_subset(str_glue("{derived_vars[23]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean") %>% 
  setNames("mean")
...
pmap(windows, function(x1,y1){
  ...
  fn_plot(x1,y1, slices = c(2,4,6), type = "div", u = "z-score")  
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[24]}_")) %>% 
  str_subset("v02") %>% 
...
pmap(windows, function(x1,y1){
  ...
  fn_plot(x1,y1, slices = c(2,4,6), type = "seq", u = "prob.")  
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[25]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "perc50") %>% 
  setNames("mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[26]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "perc50") %>% 
  setNames("mean")
...
pmap(windows, function(x1,y1){
  ...
  ggplot() +
    geom_raster(data = tb, aes(lon, lat, fill = factor(climzone))) +
    facet_wrap(~wl, ncol = 3) +
    theme(axis.title = element_blank(),
          legend.position = "bottom") +
    coord_equal() +
