...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[7]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
pmap(windows, function(x1,y1){
  ...
  fn_plot(x1,y1, type = "seq", u = "nights")  
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[8]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[9]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[10]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[11]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[12]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[13]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[14]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <- 
  dir_mosaicked %>% 
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[15]}_")) %>% 
  str_subset("v02") %>% 
  read_ncdf(var = "mean")
...
s <-
  dir_mosaicked %>%
  list.files(full.names = T) %>%
  str_subset(str_glue("{derived_vars[16]}_")) %>%
  str_subset("v02") %>%
  read_ncdf(var = "mean")
